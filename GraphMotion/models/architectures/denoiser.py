...
from GraphMotion.models.architectures.tools.embeddings import (TimestepEmbedding,
                                                               Timesteps)
from GraphMotion.models.operator import PositionalEncoding
from GraphMotion.models.operator.cross_attention import (SkipTransformerEncoder,
                                                         TransformerDecoder,
                                                         TransformerDecoderLayer,
                                                         TransformerEncoder,
                                                         TransformerEncoderLayer)
from GraphMotion.models.operator.position_encoding import build_position_encoding
from GraphMotion.utils.temos_utils import lengths_to_mask
...
class Denoiser(nn.Module):
    ...
    def __init__(self,
        ...
        self.large_encoder_proj = nn.Sequential(
            nn.Linear(256, self.latent_dim),
            nn.ReLU(),
            nn.Linear(self.latent_dim, self.latent_dim))

        self.large_decoder_proj = nn.Sequential(
            nn.Linear(self.latent_dim, self.latent_dim),
            nn.ReLU(),
    ...
    def forward(self,
        ...
        sample = self.large_encoder_proj(sample)
        ...
        if self.condition in ["text", "text_uncond"]:
            ...
            stage = encoder_hidden_states["stage"]
            if stage == 1:
                new_cond_emb = encoder_hidden_states["new_cond_emb"].permute(1, 0, 2)
                text_emb = new_cond_emb
            ...
            if stage == 2:
                motion_emb = self.large_encoder_proj(encoder_hidden_states["z"])
                new_cond_emb = encoder_hidden_states["new_cond_emb"].permute(1, 0, 2)
                new_act_emb = encoder_hidden_states["new_act_emb"].permute(1, 0, 2)
                text_emb = torch.cat([new_cond_emb, new_act_emb], dim=0)

                act_mask = encoder_hidden_states["act_mask"]
                if self.abl_plus:
                    enc_mask = torch.cat([torch.ones((act_mask.size(0), 7)).to(act_mask.device), act_mask], dim=1)
                else:
                    enc_mask = torch.cat([torch.ones((act_mask.size(0), 8)).to(act_mask.device), act_mask], dim=1)
                ...
                if self.text_encoded_dim != self.latent_dim:
                    ...
                    text_emb_latent = text_emb
                text_emb_latent = torch.cat([motion_emb, text_emb_latent], dim=0)

            if stage == 3:
                motion_emb = self.large_encoder_proj(encoder_hidden_states["z"])
                new_cond_emb = encoder_hidden_states["new_cond_emb"].permute(1, 0, 2)
                new_act_emb = encoder_hidden_states["new_act_emb"].permute(1, 0, 2)
                new_ent_emb = encoder_hidden_states["new_ent_emb"].permute(1, 0, 2)
                text_emb = torch.cat([new_cond_emb, new_act_emb, new_ent_emb], dim=0)

                act_mask = encoder_hidden_states["act_mask"]
                ent_mask = encoder_hidden_states["ent_mask"]
                if self.abl_plus:
                    enc_mask = torch.cat([torch.ones([act_mask.size(0), 13]).to(act_mask.device), act_mask, ent_mask], dim=1)
                else:
                    enc_mask = torch.cat([torch.ones([act_mask.size(0), 14]).to(act_mask.device), act_mask, ent_mask], dim=1)
        ...
        if self.arch == "trans_enc":
            ...
            if enc_mask is not None:
                enc_mask = enc_mask.unsqueeze(1).expand(-1, enc_mask.size(1), -1)
                enc_mask = enc_mask.masked_fill(enc_mask == 0, float('-inf')).masked_fill(enc_mask == 1, float(0.0))
            tokens = self.encoder(xseq, mask=enc_mask)
        ...
        sample = self.large_decoder_proj(sample)
