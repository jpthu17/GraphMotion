import inspect
import os
from GraphMotion.transforms.rotation2xyz import Rotation2xyz
import numpy as np
import torch
...
import time
from GraphMotion.config import instantiate_from_config
from os.path import join as pjoin
from GraphMotion.models.architectures import (
    denoiser,
    vae,
    vposert_vae,
    t2m_motionenc,
...
from GraphMotion.models.losses.GraphMotion import GraphMotionLosses
from GraphMotion.models.modeltype.base import BaseModel
from GraphMotion.utils.temos_utils import remove_padding
from GraphMotion.models.architectures.gat import GATLayer
...
class GraphMotion(BaseModel):
    ...
    def __init__(self, cfg, datamodule, **kwargs):
        ...
        if self.stage == "diffusion":
            if self.vae_type in ["GraphMotion", "vposert", "actor"]:
        ...
        if self.stage == "diffusion":
            self.gat = GATLayer()
            self.denoiser_stage1 = instantiate_from_config(cfg.model.denoiser)
            self.denoiser_stage2 = instantiate_from_config(cfg.model.denoiser)
            self.denoiser_stage3 = instantiate_from_config(cfg.model.denoiser)
        ...
        if cfg.LOSS.TYPE == "GraphMotion":
            self._losses = MetricCollection({
                split: GraphMotionLosses(vae=self.is_vae, mode="xyz", cfg=cfg)
                for split in ["losses_train", "losses_test", "losses_val"]
            })
        else:
            raise NotImplementedError(
                "MotionCross model only supports GraphMotion losses.")
    ...
    def forward(self, batch):
        ...
        with torch.no_grad():
            ...
            if self.vae_type in ["GraphMotion", "actor"]:
    ...
    def test_diffusion_forward(self, batch, finetune_decoder=False):
        ...
        with torch.no_grad():
            if self.vae_type in ["GraphMotion", "vposert", "actor"]:
                ...
                raise TypeError("vae_type must be mcross or actor or GraphMotion")
    ...
    def t2m_eval(self, batch):
        ...
        if self.stage in ['diffusion', 'vae_diffusion']:
            ...
            if self.do_classifier_free_guidance:
                z_stage1, z_stage1_uncond = self._diffusion_reverse(cond_emb, lengths, stage=1,
                                                                    hidden_states=cond_emb_stage1)
                z_stage1_all = torch.cat([z_stage1_uncond, z_stage1], dim=1)  # 1, 64, 256
            else:
                z_stage1 = self._diffusion_reverse(cond_emb, lengths, stage=1, hidden_states=cond_emb_stage1)
            ...
            if self.do_classifier_free_guidance:
                z_stage2, z_stage2_uncond = self._diffusion_reverse(cond_emb, lengths, stage=2,
                                                                    hidden_states=cond_emb_stage2)
            ...
            if self.do_classifier_free_guidance:
                z_stage3, z_stage3_uncond = self._diffusion_reverse(cond_emb, lengths, stage=3,
                                                                    hidden_states=cond_emb_stage3)
            else:
                ...
                if metric == "TemosMetric":
                    ...
                    if self.stage == "vae":
                        ...
                        getattr(self, metric).update(
                            ...
                            # rs_set['lat_t'], rs_set['lat_rm'], rs_set['lat_m'], batch["length"])
                            rs_set["lat_t"],
                            rs_set["lat_rm_stage1"] * self.sum_scale[0] + rs_set["lat_rm_stage2"] * self.sum_scale[1] +
                            rs_set["lat_rm_stage3"] * self.sum_scale[2],
                            rs_set["lat_m"],
                            batch["length"],
                        )
