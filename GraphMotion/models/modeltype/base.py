...
from GraphMotion.models.metrics import ComputeMetrics, MRMetrics, TM2TMetrics, MMMetrics, HUMANACTMetrics, UESTCMetrics, UncondMetrics
    ...
    def configure_metrics(self):
        for metric in self.metrics_dict:
            if metric == "TemosMetric":
                ...
                self.TM2TMetrics_stage1 = TM2TMetrics(
                    diversity_times=30
                    if self.debug else self.cfg.TEST.DIVERSITY_TIMES,
                    dist_sync_on_step=self.cfg.METRIC.DIST_SYNC_ON_STEP,
                    stage=1
                ...
                self.TM2TMetrics_stage2 = TM2TMetrics(
                    diversity_times=30
                    if self.debug else self.cfg.TEST.DIVERSITY_TIMES,
                    dist_sync_on_step=self.cfg.METRIC.DIST_SYNC_ON_STEP,
                    stage=2
                ...
                self.TM2TMetrics_stage3 = TM2TMetrics(
                    diversity_times=30
                    if self.debug else self.cfg.TEST.DIVERSITY_TIMES,
                    dist_sync_on_step=self.cfg.METRIC.DIST_SYNC_ON_STEP,
                    stage=3
                ...
                self.MMMetrics_stage1 = MMMetrics(stage=1)
            elif metric == "MMMetrics_stage2":
                self.MMMetrics_stage2 = MMMetrics(stage=2)
            elif metric == "MMMetrics_stage3":
                self.MMMetrics_stage3 = MMMetrics(stage=3)
