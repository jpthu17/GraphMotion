...
# from GraphMotion.datasets.get_dataset import get_datasets
from GraphMotion.data.get_data import get_datasets
from GraphMotion.data.sampling import subsample, upsample
...
def main():
    ...
    if cfg.DEMO.RENDER:
        # plot with lines
        # from GraphMotion.data.humanml.utils.plot_script import plot_3d_motion
        # fig_path = Path(str(npypath).replace(".npy",".mp4"))
        # plot_3d_motion(fig_path, joints, title=text, fps=cfg.DEMO.FRAME_RATE)

        # single render
        # from GraphMotion.utils.demo_utils import render
